...
| tanggal lahir di form guru | (commit ini) | **WAJIB salin ulang: `Murid.gs`, `js_murid.html`** — laporan pemilik: belum ada cara memasukkan tanggal lahir (dibutuhkan login WA §5.8 & pemulihan akses §5.5). Form Tambah/Edit Murid kini punya isian **Tanggal lahir** (berdampingan rombel); `Murid.simpan` menanganinya di 2 jalur (normalisasi `Util.tglLahirSah` — terima dd/mm/yyyy; asing ditolak VALIDASI_GAGAL di buat MAUPUN edit; kosong = boleh); `daftar`/`detail` membawa `tanggal_lahir` (Date dari Sheets → string YYYY-MM-DD); mock pratinjau ikut (siswa02 = 2008-08-08, detail tak lagi angka pasti) | uji-murid 65→71, alur UI 87→89 ✔ |
| course beranda langsung terbuka | (commit ini) | **WAJIB salin ulang: `js_beranda.html`** — laporan pemilik: klik course di daftar "Course saya" (beranda) sempat mampir ke kisi Kelola Course dulu; kini klik course **AKTIF → LANGSUNG buka course** (layar Kelola Topik & Item, `course_detail`); course nonaktif → toast pengingat mengaktifkan lewat Kelola Course (konsisten kisi: nonaktif tanpa Buka); tombol "Kelola semua" & "Buat course pertama" tetap ke kisi | alur UI +0 (1 diubah) → 89 ✔ |
| **Tahap 4 poin 2a — editor Materi** | (commit ini) | **WAJIB salin ulang: `Topik.gs`, `Code.gs`, `js_course.html`, `js_core.html`, `css.html`** — klik item **Materi** (atau ✏️-nya) di susunan → **layar editor** (rute baru `item_editor`, pola layar penuh editor v1): judul/deskripsi/status + **toolbar 13 alat `execCommand`** (B/I/U, H3/H4/¶, daftar •/1., kutip, 🔗 tautan, 🖼 gambar URL, ▶ YouTube — ID+penanda waktu diporting v1, sematan youtube-nocookie, </> kode) + contenteditable dgn tipografi; simpan = `courseUbahItem` +`konten` (server: **`Util.sanitasi`** — `<script>` dibuang, iframe youtube-nocookie dipertahankan, `javascript:` dinetralkan; jenis non-materi ditolak "menyusul 2b–2d"); endpoint baru **`courseAmbilKonten`** (`Topik.ambilKontenItem`: data editor+ta_id); item quiz/tugas/refleksi klik → toast "menyusul"; `dialogUbahItem` dihapus (digantikan layar editor); bug tertangkap uji: cabang `!aksi` me-return sebelum cek item → editor tak pernah terbuka; mock ikut (seed konten pecahan) | uji-topik 47→55, alur UI 89→93 ✔, uji-ui & extra ✔ |
| **Tahap 4 poin 2b — editor Quiz** | (commit ini) | **WAJIB salin ulang: `Setup.gs`, `Quiz.gs` (BARU), `Code.gs`, `js_course.html`, `css.html`** — klik item **Quiz** (atau ✏️-nya) di susunan → layar editor quiz (rute `item_editor`, dispatcher `renderEditor` per jenis): **backend `Quiz.gs`** — `muat` LAZY (baris `Quizzes` dibuat saat pertama dibuka, tanpa baris yatim; default `max_attempts 1`, `kkm 75`, `acak_soal`/`acak_opsi` true, `show_score` true, `tampilkan_pembahasan` false, status aktif), `simpanPengaturan` (tenggat `YYYY-MM-DD[ HH:MM]` boleh kosong, kesempatan 0–99, KKM 0–100), `simpanSoal` 4 tipe v1 (pg: 2–5 opsi + kunci huruf dalam rentang; benar_salah: Benar/Salah; isian: kunci wajib ≤100 kar; esai: tanpa kunci + rubrik ≤1000; pertanyaan ≤2000, bobot 1–100, gambar `https?://` ≤500, tingkat C1–C6; soal baru selalu dasar `order_no=N+1`), `hapusSoal`+renumber, `pindahSoal` tukar tetangga; **`answer_key`/`rubric`/`pembahasan` hanya utk guru** (`_bentukSoal(s,utkGuru)` — endpoint murid nanti WAJIB `utkGuru=false`); guard kepemilikan via topik→`ta_id` + tolak jenis non-quiz; **5 endpoint guru `quiz*`** di `Code.gs`; **UI `js_course.html`**: kartu pengaturan (tenggat/kesempatan/KKM/nilai/acak soal/acak opsi/pembahasan), strip rekap (jml soal · total bobot · per tipe · ⚠ ada uraian), daftar soal (badge nomor, tipe·bobot·tingkat, ▲▼✏️🗑 nonaktif di ujung, opsi A–E + sorot benar saat 🔑, kunci + pembahasan), dialog soal per tipe (radio kunci PG, Benar/Salah, kunci isian, rubrik esai); skema: `Quizzes` +`kkm`/`acak_soal`/`acak_opsi`/`tampilkan_pembahasan`, `Quiz_Questions` +`tingkat`(C1–C6)/`pembahasan`/`gambar_url` (paritas v1 di luar §7.10/7.11 — disetujui pemilik); bug tertangkap uji alur: seed mock memakai kunci `itm3` padahal id item `itm-3` → rekap 0 soal; kelas util CSS (`baris-antara`/`lembut`/`mb-8`) ditambahkan; hook dialog `[data-qz-opsi]` | uji-quiz **33/33 ✔** (baru), alur UI 93→**99 ✔** (+6 quiz), uji-ui & extra ✔ |
| pembahasan tampil di 🔑 Lihat kunci | (commit ini) | **WAJIB salin ulang: `js_course.html`** — saat kunci dibuka, kartu **Uraian (esai)** kini menampilkan **Rubrik** + **Pembahasan** (sebelumnya kosong sama sekali — dibatasi `type !== 'esai'`); pg/benar_salah/isian sudah menampilkan “Kunci: … · Pembahasan: …” sejak 2b | alur UI 99→**100 ✔** (+1 cek) |
| dialog soal: validasi di tempat, isian tak hilang | (commit ini) | laporan pemilik: soal bergambar "tidak muncul" di bank soal — akarnya: saat server menolak (URL gambar tanpa `http(s)://`, PG <2 opsi, dsb.) dialog hanya menampilkan toast lalu terbuka ulang **KOSONG** — semua ketikan hilang. Kini: **validasi klien `saatYa`** (cermin aturan `Quiz.simpanSoal`: pertanyaan wajib ≤2000, gambar `http(s)://` ≤500, PG ≥2 opsi terisi + kunci dalam opsi terisi, kunci isian wajib ≤100, rubrik ≤1000, bobot 1–100) — dialog **TETAP TERBUKA** + pesan di dalam dialog (`#salah-dialog-qz`, `.pesan-salah.tampil`); bila server tetap menolak, dialog dibuka ulang **dengan isian terakhir** (`dialogSoalQz(s, p)`); mock pratinjau ikut memvalidasi URL gambar seperti produksi | alur UI 100→**102 ✔** (+2 cek: URL ditolak di tempat & isian utuh; URL dibetulkan → soal masuk dgn gambar) |
| gambar soal: pratinjau live, Drive otomatis, gagal-muat tak senyap | (commit ini) | laporan pemilik: "preview soal tidak menampilkan gambar" — penyebab lazim: tautan **bukan berkas gambar langsung** (tautan Drive `/file/d/…/view` / halaman web) atau `http://` diblokir halaman https. Kini: **(1)** tautan Google Drive (`/file/d/ID`, `open?id=`, `uc?id=`) dikonversi otomatis ke endpoint thumbnail resmi saat render; **(2)** dialog soal punya **pratinjau gambar live** — thumbnail muncul seketika saat menempel tautan, gagal → kotak merah penjelasan di dalam dialog; **(3)** `http://` dinaikkan ke `https://` saat simpan (halaman https menolak gambar http); **(4)** gambar di kartu yang gagal dimuat diganti **chip peringatan** (delegasi event `error` capture di `#qz-daftar-soal`), bukan hilang senyap; css += `.qz-gambar-pratinjau`/`.qz-gambar-gagal`; jsdom tak memuat sumber daya → uji kegagalan via dispatch event `error` | alur UI 102→**105 ✔** (+3 cek: pratinjau live dgn Drive, kartu render thumbnail, chip gagal-muat) |
| gambar dikirim tanpa Referer (hotlink protection) | (commit ini) | laporan pemilik: tautan langsung `.jpg` (aaha.org) tetap tak tampil — pola khas **hotlink protection**: URL dibuka di address bar BOLEH (tanpa Referer) tapi sematan dihalaman lain DITOLAK (Referer = situs kita). Kini seluruh halaman mengirim gambar **tanpa Referer**: `<meta name="referrer" content="no-referrer">` di `index.html` (+ kepala pratinjau) dan `referrerpolicy="no-referrer"` pada `<img>` quiz (dialog + kartu); pesan galat gambar (dialog & chip) kini menyarankan **unggah ke Google Drive → tempel tautannya** bila situs memblokir sematan; jsdom tak punya properti `referrerPolicy` → uji baca `getAttribute` | alur UI 105→**106 ✔** (+1 cek) |
| **2b.x — upload gambar (📤 Unggah + tempel URL)** | (commit ini) | keputusan pemilik: dua jalur berdampingan, dikerjakan sebelum 2c. **Backend `Gambar.gs` (BARU)** — `unggah(sesi,p)`: mime whitelist JPG/PNG/GIF/WebP, base64 (awalan `data:` dibersihkan), maks 3 MB, disimpan sbg berkas Drive di folder lazy **"LMS v2 — Gambar Soal"** (id tercache di `FOLDER_GAMBAR_ID`), `setSharing(ANYONE_WITH_LINK, VIEW)` supaya murid tanpa login Google tetap lihat (domain pemblokir → galat jelas `DRIVE_BLOKIR`), kolom `gambar_url` diisi `https://drive.google.com/file/d/ID/view` (dirender thumbnail oleh konversi yang sudah ada); audit `GAMBAR_UNGGAH`; endpoint **`gambarUnggah`** (guru). **UI**: tombol 📤 di dialog soal → `FileReader` → kompres browser (maks 1280 px, JPEG q.82; dilewati bila canvas tak ada — jsdom) → unggah → isian tautan terisi otomatis + pratinjau live; mock pratinjau menyimpan dataURL di memori dan `quizMuat` memetakannya kembali — gambar hasil unggah TERLIHAT di 8080 tanpa internet; mock.js += DriveApp tiruan (folder/berkas/sharing) + `Utilities.newBlob/base64Decode` | uji-gambar **14/14 ✔** (baru), alur UI 106→**108 ✔** (+2 cek: unggah→tautan terisi→soal bergambar; PDF ditolak di dialog) |
| berbagi diblokir domain → gambar disajikan lewat web app | (commit ini) | laporan pemilik (pemakaian nyata): unggah gagal “Drive menolak berbagi tautan (kemungkinan kebijakan domain)” — domain Google Workspace lembaga memblokir `setSharing(ANYONE_WITH_LINK)` (uji repro dibuat dulu di `uji-gambar`). Kini: kegagalan berbagi **BUKAN kegagalan unggah** — berkas tetap tersimpan privat, `Gambar.unggah` mengembalikan `berbagi:false` + tautan fallback **web app sendiri** `…/exec?gambar=ID` (`ScriptApp.getService().getUrl()`); **`doGet` baru melayani `?gambar=`** via `Gambar.sajikan(id)` (publik, tanpa sesi: murid tanpa login Google tetap lihat; id divalidasi `^[A-Za-z0-9_-]+$`; mime dipetakan JPEG/PNG/GIF; id tak dikenal → teks penjelasan) — berjalan “as Me” sehingga **bebas kebijakan berbagi domain**; toast unggah membedakan mode (Drive vs disajikan lewat aplikasi); tautan Drive (bila berbagi diizinkan) tetap konversi thumbnail spt sebelumnya; mock += `ScriptApp`/`ContentService` tiruan, saklar `__setSharingGagal`, `getFileById`/`getBlob` | uji-gambar 14→**19/19 ✔** (+5: repro blokir, tautan fallback, sajikan OK/id-asing/tak-sah) |
| gagal simpan Drive terdiagnosis + kode asli tampil | (commit ini) | lanjutan laporan pemilik: “Drive menolak menyimpan berkas” (langkah `createFile`, bukan berbagi). `Gambar.unggah` kini membungkus `_folder()`+`createFile` dan **mengklasifikasi pesan asli Drive**: auth/scope → arahkan SETUJUI izin Drive saat Apps Script meminta; quota/storage → kuota penuh; disabled/sdk/policy/forbidden → Drive dimatikan admin domain; pesan selalu mengakhiri dgn **(Kode Drive: …)** agar laporan lanjutan pasti; kegagalan ikut ter-audit (`GAMBAR_UNGGAH … gagal`); catatan: **Drive API advanced service TIDAK perlu diaktifkan** — yang dipakai `DriveApp` (layanan bawaan); mock += saklar `__createFileGagal`/`__folderGagal` (getFoldersByName ikut gagal — tanpa otorisasi semua panggilan Drive gagal) | uji-gambar 19→**23/23 ✔** (+4: kuota, izin, folder-gagal, pulih) |
| pemandu izin Drive `izinDrive()` | (commit ini) | laporan pemilik terkonfirmasi lewat kode asli: `You do not have permission to call DriveApp.getFoldersByName. Required permissions: …/auth/drive` — scope Drive belum disetujui sejak `Gambar.gs` ditambah (deployment lama memakai otorisasi lama). **`izinDrive()`** (Code.gs) — helper tanpa sesi: menjalankan `getFoldersByName` sekali dari editor untuk MEMUNCULKAN dialog otorisasi; hasilnya teks `OK — izin Drive aktif…` / `BELUM AKTIF: <pesan asli>`; README += langkah 3 tahap (Run izinDrive → setujui → **deploy ulang New version** → coba unggah) | uji-gambar 23→**26/26 ✔** (+3: OK & BELUM AKTIF dgn pesan asli; hasil kini di-`Logger.log` — laporan pemilik: Execution log editor kosong karena sebelumnya hanya `return`) |
| **`appsscript.json` dikirim ke repo** (akar masalah izin) | (commit ini) | log pemilik: `izinDrive` → `BELUM AKTIF: … Required permissions (drive.readonly || drive)` dan dialog izin TIDAK pernah muncul — pola khas manifest dgn `oauthScopes` EKSPLISIT tanpa `drive` (README-TAHAP-2 memang menyuruh manifest eksplisit; versi yang disalin pemilik belum memuat drive). **`v2/appsscript.json`** kini berkas resmi di repo (spreadsheets, **drive**, external_request, scriptapp; webapp ANYONE_ANONYMOUS as-Me; tz Asia/Jakarta); README: langkah pasang += salin manifest sejak awal; cabang pemecahan masalah izin ditambah | — (docs/skrip konfigurasi; uji node tak tersentuh) |
| unggah gambar: tautan SELALU lewat web app (?gambar=ID) | (commit ini) | laporan pemilik: unggah berhasil tapi gambar tak tampil di pratinjau soal — tautan lama masih `file/d/ID/view` yang dirender via thumbnail Drive, dan di domain lembaga thumbnail ikut dibatasi. Kini `Gambar.unggah` **selalu** menyimpan `ScriptApp.getService().getUrl() + '?gambar=ID'` (berjalan as-Me, bebas kebijakan berbagi/thumbnail); `setSharing` tetap dicoba sekadar (hasil hanya di report `berbagi` utk toast); validasi klien menerima diawali `http(s)://` **atau `/`** (tautan web-app hasil unggah); mock pratinjau menyusul (tautan simulasi `/?gambar=ID` + pemetaan balik ke dataURL di `quizMuat`) | uji-gambar 26/26 ✔ (kontrak baru), alur UI **109 ✔** |
| adopsi tautan Drive lama → penyajian aplikasi | (commit ini) | lanjutan laporan “gambar tersimpan di Drive tapi tak tampil di bank soal”: **soal lama** (pra-perbaikan) masih menyimpan tautan `file/d/ID/view` yang di domain pembatas tak terrender. **`Gambar.adopsi(sesi, tautan)`** + endpoint **`gambarAdopsi`** (guru): terima tautan/id Drive, verifikasi berkas ada (`getFileById`), balik tautan web app `?gambar=ID`; **UI**: isian gambar berisi tautan Drive → tombol **📦 Sajikan lewat aplikasi** tampil di dialog → klik → tautan diganti otomatis → pratinjau live → simpan; mock pratinjau `gambarAdopsi` (+uji alur 2 cek); mock.js: berkas tiruan += `getName` (dipakai verifikasi adopsi) | uji-gambar 26→**30/30 ✔** (+4), alur UI **111 ✔** (+2) |
| pesan galat gambar dialog terdiagnosis per kasus | (commit ini) | laporan pemilik: pesan merah seragam tak membedakan penyebab. Kini `error` pratinjau live mendiagnosis dari isi isian: **tautan Drive** → “klik 📦 Sajikan lewat aplikasi di bawah”; **`?gambar=`** → arahkan Deploy New version (Gambar.gs+Code.gs) + tautan **uji buka di tab baru** (bila tampil di sana → cukup hard-refresh); lainnya → nasihat umum | alur UI 111→**113 ✔** (+2 cek pesan) |
| penanda versi TERLIHAT (login+sidebar) · APP_VERSI 2.0.1 | (commit ini) | **WAJIB salin ulang: `Code.gs`, `js_core.html`, `v_login.html`, `css.html`** — pelajaran dari laporan "tetap tidak tampil": penanda console hilang bila F12 dibuka setelah halaman loading. Kini versi TAMPIL DI LAYAR: kartu login `server v… · klien 2026-09-10`, sidebar `v… · klien 2026-09-10`; `APP_VERSI` server naik ke **2.0.1** — panduan baca: tulisan `klien 2026-09-10` tidak ada = **js_core lama**; angka server masih 2.0.0 = **Code.gs lama/New version belum**; css += `.versi-klien` | alur UI 115→**117 ✔** |
| penanda versi klien + chip wajib di kartu | (commit ini) | lanjutan laporan "gambar tetap tak muncul di bank soal": dengan klien baru, kartu bergambar **tidak mungkin kosong senyap** (berhasil = gambar, gagal = chip ⚠️) — bukti: soal dgn id gambar tak dikenal DISIMPAN → kartu menampilkan chip. Kemungkinan besar penyebab laporan: **js_course.html lama masih jalan** (belum tersalin / cache iframe GAS / belum hard-refresh). Untuk diagnosis remot: `js_core.html` kini mencetak **`[LMS v2] klien 2026-09-10 …`** ke Console (F12) — bila tak ada, browser masih memuat JS lama. **WAJIB salin ulang: `js_core.html`** (+ `js_course.html` bila versi kemarin belum) + Deploy New version + **Ctrl+Shift+R** | alur UI 114→**115 ✔** |
| **gambar: HTML utk tab baru + base64 dalam app** | (commit ini) | **WAJIB salin ulang: `Gambar.gs`, `Code.gs`, `js_course.html`** — laporan pemilik: membuka tautan `?gambar=ID` menampilkan **deretan byte** (`-1,-40,…` = JPEG). Akar: `ContentService` GAS **tidak bisa melayani byte biner** — `createTextOutput` hanya menerima string, array byte ter-coerce jadi teks angka. Dua jalur perbaikan: (1) `Gambar.sajikan` kini mengembalikan **halaman HTML** berisi `<img src="data:image/…;base64,…">` — uji tab baru tetap menampilkan gambarnya; (2) di dalam aplikasi gambar tidak lagi dipakai sbg `src` — endpoint baru **`gambarMuat`** (guru & murid, `apa_saja`) mengirim `{mime, base64}`; kartu bank soal memakai `data-srcqz` + `pasangGambarQz` (cache per id), pratinjau dialog `?gambar=` juga data URI; id tak dikenal → pesan "tidak dapat dimuat" (bukan senyap); mock: `HtmlService` + `Utilities.base64Encode` | uji-gambar 30→**34 ✔** (sajikan HTML + muatBase64 + endpoint murid), alur UI 113→**114 ✔** |
| **ralat `sajikan`: createTextOutput** | (commit ini) | laporan pemilik: `TypeError: ContentService.createOutput is not a function (Gambar.gs:137)` — nama metode yang benar adalah **`createTextOutput`**; mock lama justru mendefinisikan nama yang salah sehingga uji node tidak menangkapnya (pelajaran: mock harus mencerminkan API asli, bukan kode penelepon — kini mock memakai `createTextOutput` dgn penanda komentar); 3 pemakaian di `Gambar.sajikan` diperbaiki | uji-gambar **30/30 ✔** (mock dgn nama API asli) |
