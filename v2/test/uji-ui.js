 ...
 *
 * Latar: bug "dialogTopik is not defined" (tahap 4) lolos semua
 * uji node karena uji lama hanya menyentuh kode server (.gs).
 * Uji ini memindai SEMUA partial UI di v2/ (js_*.html, v_*.html,
 * dan <script> inline di index.html) secara statis: kumpulkan
 * semua `function nama(` sebagai definisi global, lalu semua
 * `nama(` sebagai pemanggilan (termasuk isi atribut onclick=),
 * dan laporkan yang dipanggil tapi tidak didefinisikan di
 * berkas mana pun — dengan tokenizer string/komentar/regex
 * yang benar.
 *
 * Daftar berkas dideteksi OTOMATIS dari isi v2/, jadi uji ini
 * tetap sah setiap kali susunan partial berubah (tahap baru).
 *
 * Jalankan:  node v2/test/uji-ui.js
 * ============================================================ */
'use strict';
const fs = require('fs');
...
const BUILTIN = new Set([
  ...
  'console', 'Error', 'RegExp', 'fetch', 'FormData', 'Blob', 'URL',
...
/* ---------- tokenizer: buang komentar & isi string/regex ----------
   Regex-lawan dibedakan dari tanda bagi dengan melihat token
   sebelumnya: setelah `(`, `,`, `=`, `:` dst. atau kata kunci
   (return/typeof/…) maka `/` memulai regex. */
const KEYWORD_SEBELUM_REGEX = new Set([
  'return', 'typeof', 'case', 'in', 'of', 'delete', 'void', 'new',
  'do', 'else', 'instanceof', 'throw'
]);

function tokenizeKode(sumber) {
  const out = [];
  let prev = '';       // karakter non-spasi terakhir yang keluar
...
/* ---------- kumpulkan berkas partial UI secara otomatis ---------- */
const berkas = fs.readdirSync(DIR)
  .filter(function (f) { return /\.html$/.test(f); })
  .sort();

/* Isi JS sebuah berkas = gabungan blok <script> (index.html punya
   skrip inline; js_*.html seluruhnya dibungkus <script>; berkas
   tanpa <script> — mis. css.html — berarti tidak punya JS). */
function ambilSkrip(mentah) {
  return (mentah.match(/<script[^>]*>([\s\S]*?)<\/script>/g) || [])
    .join('\n').replace(/<\/?script[^>]*>/g, '\n');
}

/* ---------- definisi global & nama parameter ---------- */
const definisi = new Set();
const parameter = new Set();  // param fungsi/callback — bukan kandidat global

for (const f of berkas) {
  const kode = tokenizeKode(ambilSkrip(fs.readFileSync(path.join(DIR, f), 'utf8')));
  let m;
...
function onclickHtml(mentah, label) {
  let m;
  const re = /on(?:click|change|submit|input|load)\s*=\s*"([^"]*)"/g;
  while ((m = re.exec(mentah))) catatPakai(m[1], label + ':onclick');
}

for (const f of berkas) {
  const mentah = fs.readFileSync(path.join(DIR, f), 'utf8');
  catatPakai(tokenizeKode(ambilSkrip(mentah)), f);
...
console.log('Berkas UI dipindai: ' + berkas.length +
            ' · definisi global: ' + definisi.size);
if (!daftar.length) {
  ...
  console.log('');
  console.log('UJI UI GAGAL ✘ — ' + gagal +
              ' fungsi hilang (ReferenceError di browser).');
  process.exit(1);
}
