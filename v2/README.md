...
| `index.html` + `css.html` | Cangkang UI; **scriptlet `<? ?>` hanya di index.html** (berkas include tak dievaluasi scriptlet); `css.html` = sistem desain bergaya v1 (palet KESEPAKATAN-SISTEM.md §1: `--hijau #7BC96F`, latar `#F6FBF5`, Plus Jakarta Sans, tombol sentuh 44px; ganti tema cukup ubah `:root`; responsif 4 breakpoint: 1024/900/700/380 — ponsel: sidebar off-canvas) |
...
| `Gambar.gs` | **Tahap 4 poin 2b.x** — unggah gambar (soal quiz; nanti materi): mime JPG/PNG/GIF/WebP, maks 3 MB (klien kompres dulu ±1280 px), berkas di folder Drive **"LMS v2 — Gambar Soal"** (lazy, id di `FOLDER_GAMBAR_ID`), dibagikan siapa-saja-dengan-tautan agar murid tanpa login Google bisa melihat; kolom `gambar_url` berisi tautan **web app sendiri** `…/exec?gambar=ID` (disajikan doGet as-Me — bebas kebijakan berbagi/thumbnail domain; **GAS tak bisa mengirim byte biner**, jadi buka tautan di tab = halaman HTML `<img>` data-URI, dan di dalam aplikasi gambar dimuat via `gambarMuat` (base64) |
| `gambarAdopsi` | Tautan Drive lama pada soal lama → penyajian aplikasi; di UI muncul tombol 📦 saat isian berisi tautan Drive |
| `gambarMuat` | Isi gambar untuk `<img>` di dalam aplikasi (guru & murid) → `{mime, base64}`; kartu soal & pratinjau dialog memakainya sbg data URI |
...
| `test/` | Uji node: `uji-auth-gate0.js` (18) + `uji-murid.js` (65) + `uji-lupa-akses.js` (35) + `uji-kelas.js` (55) + `uji-course.js` (41) + `uji-quiz.js` (33) + `uji-gambar.js` (34) + `uji-apikey.js` (31) + `uji-beranda.js` (18) + `uji-ui.js` (audit statis fungsi klien; daftar berkas auto) + `uji-ui-extra.js` (audit id/endpoint/CSS/rute) + `uji-alur-ui.js` (opsional: alur klik di jsdom, dilewati bila jsdom/pratinjau tak ada) |
